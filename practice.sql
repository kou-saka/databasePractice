...
SELECT SUBSTRING(code, 1, 1) AS code_initial,
       name
FROM countries;
...
FROM countries           AS c
JOIN country_languages   AS cl  ON c.code = cl.country_code
JOIN cities              AS ci  ON c.code = ci.country_code;
...
SELECT
  ...
  (
    SELECT name
    FROM countries
    WHERE countries.code = ce.country_code
  ) AS country_name
FROM celebrities AS ce;
...
-- 問31
-- 1991年生まれと、1981年生まれの有名人が何人いるか調べてください。
-- 日付関数は使わず、UNION句を使用します。
SELECT '1991年' AS birth_year, COUNT(*) AS celebrity_count
FROM celebrities
WHERE birth::text LIKE '1991-%'
UNION
SELECT '1981年' AS birth_year, COUNT(*) AS celebrity_count
FROM celebrities
